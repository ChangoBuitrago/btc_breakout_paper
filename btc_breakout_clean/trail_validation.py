#!/usr/bin/env python3
"""
Experiment 3: ATR trails on crypto sleeves (live keeps trail off).

Modes (crypto only; metals/oil unchanged):
  - baseline: no trail (live)
  - trail_atr_*: legacy pct-vol trail (peak × (1 − mult × atr14))
  - trail_2n_peak: Turtle peak − 2×N (20-day true range), with live % stops kept

Run from repo root:
  .venv/bin/python btc_breakout_clean/trail_validation.py
"""
...
OUT_PATH = HERE / "trail_validation_results.json"

MODES: dict[str, dict[str, Any]] = {
    "baseline": {
        "crypto": {"trail_atr": 0.0, "trail_n_mult": 0.0},
        "note": "Live: no trail",
    },
    "trail_atr_1.0": {
        "crypto": {"trail_atr": 1.0, "trail_n_mult": 0.0},
        "note": "Close < peak × (1 − 1.0×atr14)",
    },
    "trail_atr_1.5": {
        "crypto": {"trail_atr": 1.5, "trail_n_mult": 0.0},
        "note": "Close < peak × (1 − 1.5×atr14)",
    },
    "trail_atr_2.0": {
        "crypto": {"trail_atr": 2.0, "trail_n_mult": 0.0},
        "note": "Close < peak × (1 − 2.0×atr14)",
    },
    "trail_2n_peak": {
        "crypto": {"trail_atr": 0.0, "trail_n_mult": 2.0},
        "note": "Low ≤ peak_close − 2×N; live % stops remain",
    },
}
...
def run_mode(raw: dict[str, pd.DataFrame], mode: str) -> dict[str, Any]:
    ...
    if not trades.empty and "exit_reason" in trades.columns:
        ...
        trail = crypto[crypto["exit_reason"].isin(("trail", "trail_n"))] if not crypto.empty else crypto
        metrics["trail_exit_count"] = int(len(trail))
    else:
        metrics["exit_reasons"] = {}
        metrics["trail_exit_count"] = 0
    return metrics
...
def main() -> None:
    ...
    for mode, meta in MODES.items():
        ...
        m = metrics
        trails = metrics.get("exit_reasons", {})
        print(
            ...
            f"trail_exits={m.get('trail_exit_count', 0)}  "
            f"reasons={{trail:{trails.get('trail', 0)}, trail_n:{trails.get('trail_n', 0)}}}",
            flush=True,
        )
